...
- **Fixed-block window, not rolling (FR-S1).** Codex meters its session limit in fixed 5h blocks; `Estimator.windowState` finds the current block (first activity, advancing every time a record lands past `blockStart + 5h`) and sums from there. The spec's rolling "oldest entry + 5h" model diverged from Codex's real reset time.
- **Percentage calibration (FR-C2).** The JSONL cannot yield Codex's true limit denominator — `cache_read` tokens dominate the weighted sum (often 10–20M in a block), so any fixed cap is a guess. `AppState.calibrate(observedPercentUsed:)` instead back-computes the cap from a `% used` figure the user reads off Codex's own usage screen. Until calibrated, "% left" is not trustworthy.
...
- App sandbox is **off** (`ENABLE_APP_SANDBOX = NO`) — required to read `~/.Codex/projects/` (§6.4).
...
# Codex-fuel · v0.2 Requirements (Swift / macOS)
...
| Data source | DOM scraping on Codex.ai | Local Codex JSONL transcripts |
| Token counts | Estimated (chars/3.8, ±10–15%) | Real (from `message.usage` field) |
| Target audience | Codex.ai web users | Codex Pro/Max subscribers |
| Platform | macOS, Windows, Linux | macOS only |

### 1.2 Why macOS-only Swift

**Audience reality**: Codex Pro/Max subscribers skew heavily macOS (developer demographics + Apple Silicon performance for AI workflows). Windows/Linux subset is small enough that supporting it costs more in quality than it returns in coverage.

**Design fidelity**: The product's differentiation rests on feeling Codex-native — Source Serif 4 type, terracotta accent, precise spacing, fluid SwiftUI popovers. WebView-rendered UIs are visibly second-rate on macOS to the audience we want to win.
...
A native macOS menu-bar companion for Codex that reads your local session transcripts, shows live token usage and 5-hour window remaining, and suggests when to start a fresh session before context rot makes it expensive.

### 2.2 Primary user
A Codex Pro or Max subscriber on macOS who:
- Runs `Codex` (the CLI) regularly on their machine
...
- ❌ Not a usage tracker for Codex.ai web users (no JSONL → no path)
- ❌ Not a Windows or Linux app in v0.2
- ❌ Not a billing/cost calculator
- ❌ Not a transcript viewer (lm-assist, Codex-log exist)
...
3. **Calibrated to Codex's design language** via SwiftUI Source Serif 4 + terracotta system
...
- Primary location: `~/.Codex/projects/`
- Fallback location: `~/.config/Codex/projects/`
...
- This addresses the [JSONL 10–100× undercount issue](https://gille.ai/en/blog/Codex-jsonl-logs-undercount-tokens/) that affects all naive parsers
...
- Content: "Install Codex to get started", brief 3-step quickstart, link to docs
...
- Confirmation sheet: "Use this only after Codex reported a limit hit"
...
- Unknown JSONL fields ignored (forward-compatible with Codex updates)
...
- Config storage: `~/Library/Application Support/dev.ysong.Codex-fuel/`
...
- No Mac App Store in v0.2 (sandbox restrictions on reading `~/.Codex/` are awkward)
...
- **Watcher**: dedicated `DispatchQueue(label: "Codex-fuel.watcher")` for file system events
...
- `Codex-fuel-menubar/src-tauri/` (entire Rust project)
- `Codex-fuel/` (entire Chrome extension)
...
### 6.1 Anthropic ships `Codex usage` officially
Issue [anthropics/Codex#33978](https://github.com/anthropics/Codex/issues/33978) is a feature request for an official command. If/when shipped, raw data display is commoditized.
...
Reading `~/.Codex/projects/` from a sandboxed app requires user file-access grant via `NSOpenPanel` (not transparent). Mac App Store would force sandboxing.
...
### 6.5 Multiple Codex sessions
User may have several `Codex` processes writing to different JSONL files simultaneously. All count toward the same 5h rate limit.
...
Codex writes subagent activity to `subagents/` subdirectories.
...
- [ ] Empty state appears correctly when `~/.Codex/projects/` absent
...
### 9.1 Naming
"Codex-fuel" retained — works for any Codex session context.

### 9.2 Positioning
- Hero line: *"A quiet meter for your Codex sessions."*
...
4. Submit to `awesome-Codex` and `awesome-mac` lists
...
# Where it puts data
~/Library/Application Support/dev.ysong.Codex-fuel/
  ├── settings.json
